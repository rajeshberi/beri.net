...
export default async function Home() {
  ...
  const newTools = tools
    .filter(t => t && (t.name || t.productName))
    .sort((a, b) => {
      const dateA = a.addedDate || a.discovered || a.last_updated || 0;
      const dateB = b.addedDate || b.discovered || b.last_updated || 0;
              ...
              {newTools.length > 0 && (
                <ScrollReveal>
                  <section className="pb-16">
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {newTools.filter(tool => tool && (tool.name || tool.productName)).map((tool, i) => (
                          ...
                          <Link href={`/tools/${tool.slug}`} className="group block h-full">
                            <article className="card card-glow h-full flex flex-col p-6 hover:border-purple-500/30 transition-all">
                              ...
                              <div className="flex items-start gap-4 mb-4">
                                {(tool.logoUrl || tool.logo_url) ? (
                                  <img 
                                    src={tool.logoUrl || tool.logo_url} 
                                    alt={tool.productName || tool.name}
                                  ...
                                  <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-purple-500 to-fuchsia-600 flex items-center justify-center text-white font-bold">
                                    {(tool.productName || tool.name)?.charAt(0)}
                                  </div>
                                )}
                                <div className="flex-1 min-w-0">
                                  ...
                                  <p className="text-xs text-white/40">by {tool.vendorName || tool.company}</p>
                                </div>
                              </div>
